...
var { Snarky, Ledger, Pickles, Test } = snarky;
...
function rangeCheckHelper(length, x) {
  ...
  if (x.isConstant()) {
    let bits2 = BinableFp.toBits(x.toBigInt()).slice(0, length).concat(Array(Fp.sizeInBits - length).fill(false));
    return createField2(BinableFp.fromBits(bits2));
  }
...
var Field3 = class _Field {
  ...
  assertEquals(y, message) {
    try {
      if (this.isConstant() && isConstant3(y)) {
  ...
  isOdd() {
    if (this.isConstant())
      return new Bool3((this.toBigInt() & 1n) === 1n);
    let [b2, z] = exists(2, () => {
      let x = this.toBigInt();
      return [x & 1n, x >> 1n];
    });
    let isOdd = b2.assertBool();
    z.assertLessThan((_Field.ORDER + 1n) / 2n);
    b2.add(z.mul(2)).assertEquals(this);
  ...
  toBits(length = 254) {
    ...
    if (this.isConstant()) {
      let bits3 = BinableFp.toBits(this.toBigInt());
    ...
    let bits2 = Provable.witness(Provable.Array(Bool3, length), () => {
      let f = this.toBigInt();
      return Array.from({ length }, (_, k) => new Bool3(!!(f >> BigInt(k) & 0x1n)));
    });
    _Field.fromBits(bits2).assertEquals(this, `Field.toBits(): Input does not fit in ${length} bits`);
    return bits2;
  }
  /**
  ...
  static fromBits(bits2) {
    const length = bits2.length;
    checkBitLength("Field.fromBits()", length, 254);
    if (bits2.every((b2) => typeof b2 === "boolean" || b2.toField().isConstant())) {
      let bits_ = bits2.map((b2) => typeof b2 === "boolean" ? b2 : b2.toBoolean()).concat(Array(Fp.sizeInBits - length).fill(false));
      return new _Field(BinableFp.fromBits(bits_));
    }
    return bits2.map((b2) => new Bool3(b2)).reduce((acc, bit, idx) => {
      const shift3 = 1n << BigInt(idx);
      return acc.add(bit.toField().mul(shift3));
    }, _Field.from(0)).seal();
  }
  /**
  ...
  seal() {
    ...
    if (terms.length === 0)
      return new _Field(constant);
    if (terms.length === 1 && constant === 0n) {
      let [c, x2] = terms[0];
      if (c === 1n)
        return new _Field(x2);
    }
...
var scalarShift = Fq.mod(1n + 2n ** 255n);
var oneHalf = Fq.inverse(2n);
var Scalar3 = class _Scalar {
  ...
  static from(x) {
    if (x instanceof _Scalar)
      return x;
    let x_ = ScalarConst.is(x) ? constToBigint2(x) : x;
   ...
   */
  static fromBits(bits2) {
    return _Scalar.fromFields([
      ...bits2.map((b2) => b2.toField()),
      ...Array(Fq.sizeInBits - bits2.length).fill(new Bool3(false))
    ]);
  }
  /**
   ...
   */
  static toInput(x) {
    return { packed: _Scalar.toFields(x).map((f) => [f, 1]) };
  }
  /**
   * Part of the {@link Provable} interface.
   *
...
function toConstantScalar([, ...bits2]) {
  if (bits2.length !== Fq.sizeInBits)
    throw Error(`Scalar: expected bits array of length ${Fq.sizeInBits}, got ${bits2.length}`);
...
function rotate64(field, bits2, direction = "left") {
  assert3(bits2 >= 0 && bits2 <= 64, `rotation: expected bits to be between 0 and 64, got ${bits2}`);
  if (field.isConstant()) {
    assert3(field.toBigInt() < 1n << 64n, `rotation: expected field to be at most 64 bits, got ${field.toBigInt()}`);
    return new Field3(Fp.rot(field.toBigInt(), BigInt(bits2), direction));
  }
...
function rotate32(field, bits2, direction = "left") {
  ...
  if (field.isConstant()) {
    ...
    return new Field3(Fp.rot(field.toBigInt(), BigInt(bits2), direction, 32n));
  }
...
function rot64(field, bits2, direction = "left") {
  const rotationBits = direction === "right" ? 64 - bits2 : bits2;
...
function rightShift64(field, bits2) {
  assert3(bits2 >= 0 && bits2 <= 64, `rightShift: expected bits to be between 0 and 64, got ${bits2}`);
  if (field.isConstant()) {
    assert3(field.toBigInt() < 1n << 64n, `rightShift: expected field to be at most 64 bits, got ${field.toBigInt()}`);
    return new Field3(Fp.rightShift(field.toBigInt(), bits2));
  }
...
function leftShift64(field, bits2) {
  ...
  if (field.isConstant()) {
    assert3(field.toBigInt() < 1n << 64n, `rightShift: expected field to be at most 64 bits, got ${field.toBigInt()}`);
    return new Field3(Fp.leftShift(field.toBigInt(), bits2));
  }
...
var UInt642 = class _UInt64 extends CircuitValue {
  ...
  rotate(bits2, direction = "left") {
    return new _UInt64(rotate64(this.value, bits2, direction).value);
  }
  /**
   ...
   */
  leftShift(bits2) {
    return new _UInt64(leftShift64(this.value, bits2).value);
  }
  /**
...
var UInt322 = class _UInt32 extends CircuitValue {
   ...
   */
  rotate(bits2, direction = "left") {
    return new _UInt32(rotate32(this.value, bits2, direction).value);
  }
  /**
   ...
   */
  leftShift(bits2) {
    return new _UInt32(leftShift32(this.value, bits2).value);
  }
  /**
   ...
   */
  rightShift(bits2) {
    return new _UInt32(rightShift64(this.value, bits2).value);
  }
  /**
...
function sliceField(x, maxBits, chunkSize, leftover) {
  ...
  if (leftover !== void 0) {
    ...
    for (let i2 = 0; i2 < size; i2++) {
      let bit = bits2[i2];
      bit.assertBool();
      remainingChunk = remainingChunk.add(bit.mul(1n << BigInt(i2)));
    }
...
var Signature2 = class _Signature extends CircuitValue {
  ...
  static create(privKey, msg) {
    ...
    const d = privKey.s;
    const kPrime = Scalar3.from(deriveNonce({ fields: msg.map((f) => f.toBigInt()) }, { x: publicKey.x.toBigInt(), y: publicKey.y.toBigInt() }, d.toBigInt(), "testnet"));
...
function scaleShifted(point, shiftedScalar) {
  let oneHalfGroup = point.scale(Scalar3.from(oneHalf2));
  let shiftGroup = oneHalfGroup.scale(Scalar3.from(shift2));
  return oneHalfGroup.scale(shiftedScalar).sub(shiftGroup);
}
function unshift2(shiftedScalar) {
  return shiftedScalar.sub(Scalar3.from(shift2)).mul(Scalar3.from(oneHalf2));
}
var shift2 = Fq.mod(1n + 2n ** 255n);
var oneHalf2 = Fq.inverse(2n);
...
var AccountUpdate3 = class _AccountUpdate {
  ...
  hash() {
    if (Provable.inCheckedComputation()) {
      ...
      return Field4(Test.hashFromJson.accountUpdate(JSON.stringify(json), NetworkId.toString(activeInstance.getNetworkId())));
    }
  }
...
setActiveInstance({
  ...activeInstance,
  async transaction(sender2, f) {
    return await createTransaction(sender2, f, 0);
  }
});
var Transaction = {
  fromJSON(json) {
  ...
  return {
    ...
    set(state3) {
      ...
      stateAsFields.forEach((x, i) => {
        AccountUpdate3.setValue(accountUpdate.body.update.appState[layout.offset + i], x);
      });
    },
...
class ${contract.constructor.name} extends SmartContract {
  ...
  return {
    ...
    reduce(actionLists, stateType, reduce, { state: state3, actionState }, { maxTransactionsWithActions = 32, maxActionsPerMethod = 1, skipActionStatePrecondition = false } = {}) {
...
Use the optional \`maxTransactionsWithActions\` argument to increase this number.`);
      ...
      let possibleActionsPerTransaction = Array.from({ length: maxActionsPerMethod + 1 }, (_, i) => i);
      ...
      for (let i = 0; i < maxTransactionsWithActions; i++) {
        ...
        let actionss = possibleActionsPerTransaction.map((n, i2) => {
          let type = possibleActionTypes[i2];
    ...
    async fetchActions(config) {
      let result = await fetchActions(contract.address, config, contract.self.tokenId);
      if ("error" in result) {
...
var MerkleMap = class {
  ...
  _keyToIndex(key) {
    ...
    for (let i = 0; i < keyBits.length; i++) {
      const b2 = keyBits[i] ? 1 : 0;
      n += 2n ** BigInt(i) * BigInt(b2);
    }
    return n;
  }
  // ------------------------------------------------
...
var MerkleMapWitness = class extends CircuitValue {
  ...
  computeRootAndKey(value) {
    ...
    for (let i = 0; i < bits; i++) {
      ...
      hash3 = Poseidon2.hash([left, right]);
      const bit = Provable.if(isLeft[i], Field4(0), Field4(1));
